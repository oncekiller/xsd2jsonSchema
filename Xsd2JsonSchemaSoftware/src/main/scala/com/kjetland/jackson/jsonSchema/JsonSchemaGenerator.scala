  ...
  (
    ...
    override def expectStringFormat(_type: JavaType) = {
      ...
      val minAndMaxLength:Option[MinAndMaxLength] = currentProperty.flatMap {
        p =>
          //CUSTOM look for @Custom for xs:duration and attribute fixed
          Option(p.getAnnotation(classOf[Custom])).map {
            adapter =>
              //xs:duration
            if(adapter.name()=="Duration"){
              print("ok")
              node.put("type", "string")
              node.put("pattern", "P(?:(?:[0-9]+D|[0-9]+M(?:[0-9]+D)?|[0-9]+Y(?:[0-9]+M(?:[0-9]+D)?)?)(?:T(?:[0-9]+H(?:[0-9]+M(?:[0-9]+S)?)?|[0-9]+M(?:[0-9]+S)?|[0-9]+S))?|T(?:[0-9]+H(?:[0-9]+M(?:[0-9]+S)?)?|[0-9]+M(?:[0-9]+S)?|[0-9]+S)|[0-9]+W)")
            }
            //attribute fixed
            if(adapter.name()=="fixed"){
              node.put("pattern","^"+adapter.value()+"$")
            }
          }
          //CUSTOM look for @XmlElement for attribute default
          Option(p.getAnnotation(classOf[XmlElement])).map {
            adapter =>
              if(adapter.defaultValue != "\u0000"){
                node.put("default",adapter.defaultValue)
              }
          }
          // Look for @Pattern
          ...
          Option(p.getAnnotation(classOf[XmlSchemaType])).map {
            adapter =>
            //xs:ENTITY
            if(adapter.name()=="ENTITY"){
              node.put("type" , "string")
              node.put("pattern" , "^[a-zA-Z_]([a-zA-Z0-9_]|\\.|-)*$")
            }
            //xs:ID
            if(adapter.name()=="ID"){
              node.put("type" , "string")
            ...
            //xs:NCName
            if(adapter.name()=="NCName"){
              node.put("type" , "string")
              node.put("pattern" , "^[a-zA-Z_]([a-zA-Z0-9_]|\\.|-)*$")
            }
            //xs:NMTOKEN
            if(adapter.name()=="NMTOKEN"){
              node.put("type" , "string")
              node.put("pattern" , "^[-._:A-Za-z0-9]+$")
            }
            //xs:normalizedString
            if(adapter.name()=="normalizedString"){
              node.put("type" , "string")
              node.put("pattern" , "^((((?!(\r|\t|\n|<|&)).)*|(&lt|&amp)*))*$")
            }
      ...
      currentProperty.map {
        p =>
          ...
          Option(p.getAnnotation(classOf[XmlSchemaType])).map {
            adapter =>
            //xs:hexBinary
            if(adapter.name()=="hexBinary"){
              node.put("type", "string")
              node.put("pattern", "^([0-9a-fA-F]{2})*$")
              node.remove("items")
            }
          }
      }
      ...
      currentProperty.map {
        p =>
          ...
          Option(p.getAnnotation(classOf[Custom])).map {
            adapter =>
               //xs:float
              if(adapter.name()=="float"){
                node.put("minimum" , (-1*Math.pow(2,127)).toFloat)
                node.put("maximum" , (Math.pow(2,127)).toFloat)
              }
               //xs:double
              if(adapter.name()=="double"){
                node.put("minimum" , (-1*Math.pow(2,1023)).toDouble)
                node.put("maximum" , (Math.pow(2,1023)).toDouble)
              }
          }
          ...
          Option(p.getAnnotation(classOf[Digits])).map {
            adapter =>
              if(adapter.fraction!=0){
                node.put("multipleOf",new BigDecimal("0."+"0"*(adapter.fraction-1)+"1"))
                node.put("maximum",new BigInteger("9"*adapter.integer))
              }
              else{
                node.put("multipleOf",new BigDecimal("0."+"0"*(adapter.integer-2)+"1"))
                node.put("maximum",new BigInteger("9"*adapter.integer))
              }      
          }
          
          //CUSTOM LOOK for @DecimalMin
          Option(p.getAnnotation(classOf[DecimalMin])).map {
            adapter =>
              try{
                node.put("minimum" , adapter.value().toLong)
              }
              catch{
                case e: Exception => node.put("minimum" , new BigInteger(adapter.value()))
              }    
          }
          
          //CUSTOM LOOK for @DecimalMax
          Option(p.getAnnotation(classOf[DecimalMax])).map {
            adapter =>
              try{
                node.put("maximum" , adapter.value().toLong)
              }
              catch{
                case e: Exception => node.put("maximum" , new BigInteger(adapter.value()))
              }    
          }
        ...
        p =>
          ...
          Option(p.getAnnotation(classOf[Custom])).map {
            adapter =>
              //xs:int
              if(adapter.name()=="int"){
                node.put("minimum" , -2147483648)
                node.put("maximum" , 2147483647)
              }
              //xs:long
              if(adapter.name()=="long"){
                node.put("minimum" , new BigInteger("-9223372036854775808"))
                    
                node.put("maximum" , new BigInteger("9223372036854775807"))
              }
          ...
          Option(p.getAnnotation(classOf[XmlSchemaType])).map {
            adapter =>
              ...
              //xs:dateTime
              if(adapter.name()=="dateTime"){
                node.put("type" , "string")
                node.put("pattern" , "^-?[0-9]{4}([0-9])*-(((0(1|3|5|7|8)|1(0|2))-(0[1-9]|(1|2)[0-9]|3[0-1]))|((0(4|6|9)|11)-(0[1-9]|(1|2)[0-9]|30))|(02-(0[1-9]|(1|2)[0-9])))T([0-1][0-9]|2[0-4]):(0[0-9]|[1-5][0-9]):(0[0-9]|[1-5][0-9])("+"""\"""+".[0-999])?(("+"""\"""+"+|-)([0-1][0-9]|2[0-4]):(0[0-9]|[1-5][0-9])|Z)?$")
              }
              //xs:date
              if(adapter.name()=="date"){
                node.put("type" , "string")
                node.put("pattern" , "^-?[0-9]{4}([0-9])*-(((0(1|3|5|7|8)|1(0|2))-(0[1-9]|(1|2)[0-9]|3[0-1]))|((0(4|6|9)|11)-(0[1-9]|(1|2)[0-9]|30))|(02-(0[1-9]|(1|2)[0-9])))(("+"""\"""+"+|-)([0-1][0-9]|2[0-4]):(0[0-9]|[1-5][0-9])|Z)?$")
              }
              //xs:time
              if(adapter.name()=="time"){
                node.put("type" , "string")
                node.put("pattern" , "^([0-1][0-9]|2[0-4]):(0[0-9]|[1-5][0-9]):(0[0-9]|[1-5][0-9])("+"""\"""+".[0-999])?(("+"""\"""+"+|-)([0-1][0-9]|2[0-4]):(0[0-9]|[1-5][0-9])|Z)?$")
              }
               //xs:dDay
              if(adapter.name()=="gDay"){
                node.put("type" , "string")
                node.put("pattern" , "^---((0[1-9])|((1|2)[0-9])|30|31)(("+"""\"""+"+|-)([0-1][0-9]|2[0-4]):(0[0-9]|[1-5][0-9])|Z)?$")
              }
               //xs:gMonth
              if(adapter.name()=="gMonth"){
                node.put("type" , "string")
                node.put("pattern" , "^--((0[1-9])|(1)[0-2])(("+"""\"""+"+|-)([0-1][0-9]|2[0-4]):(0[0-9]|[1-5][0-9])|Z)?$")
              }
               //xs:gYear
              if(adapter.name()=="gYear"){
                node.put("type" , "string")
                node.put("pattern" , "^-?([0-9]{4})([0-9])*(("+"""\"""+"+|-)([0-1][0-9]|2[0-4]):(0[0-9]|[1-5][0-9])|Z)?$")
              }
               //xs:gMonthDay
              if(adapter.name()=="gMonthDay"){
                node.put("type" , "string")
                node.put("pattern" , "^--(((0(1|3|5|7|8)|(1)[0|2])-((0[1-9])|((1|2)[0-9])|30|31))|(((0(4|6|9)|(11))-((0[1-9])|((1|2)[0-9])|30)))|(02-((0[1-9])|((1|2)[0-9]))))(("+"""\"""+"+|-)([0-1][0-9]|2[0-4]):(0[0-9]|[1-5][0-9])|Z)?$")
              }
               //xs:gYearMonth
              if(adapter.name()=="gYearMonth"){
                 
                node.put("type" , "string")
                node.put("pattern" , "^"+"""\"""+"-?([0-9]{4})([0-9])*-((0[1-9])|(1)[0-2])(("+"""\"""+"+|-)([0-1][0-9]|2[0-4]):(0[0-9]|[1-5][0-9])|Z)?$")
              }
              //xs:unsignedLong
              if(adapter.name()=="unsignedLong"){
                
                 
                node.put("minimum" , 0)
                node.put("maximum" , new BigInteger("18446744073709551615"))
              }
          ...
          thisObjectNode:ObjectNode =>
            ...
            Some(new JsonObjectFormatVisitor with MySerializerProvider {
              ...
              private def validationAnnotationRequired(prop: Option[BeanProperty]): Boolean = {
                prop.exists(p => p.getAnnotation(classOf[NotNull]) != null || p.getAnnotation(classOf[NotBlank]) != null || p.getAnnotation(classOf[NotEmpty]) != null  || (p.getAnnotation(classOf[XmlAttribute])!=null && p.getAnnotation(classOf[XmlAttribute]).required())|| (p.getAnnotation(classOf[XmlElement])!=null && p.getAnnotation(classOf[XmlElement]).required()))
              }
            })
        }
